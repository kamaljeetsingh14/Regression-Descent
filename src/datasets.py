...
def load_fmnist(batch_size=32):
    ...
    # Convert to tensors (same style as your MNIST loader)
    X_train = train_raw.data.unsqueeze(1).float() / 255.0
    X_train = transforms.Normalize((0.2860,), (0.3530,))(X_train)
    y_train = F.one_hot(train_raw.targets, 10).float()

    X_test = test_raw.data.unsqueeze(1).float() / 255.0
    X_test = transforms.Normalize((0.2860,), (0.3530,))(X_test)
    y_test = F.one_hot(test_raw.targets, 10).float()
